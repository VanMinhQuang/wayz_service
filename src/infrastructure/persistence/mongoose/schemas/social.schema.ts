...
@Schema({ timestamps: true }) export class Friendship {
...
@Schema({ timestamps: true }) export class Group {
...
@Schema({ timestamps: true }) export class GroupMember {
...
@Schema({ timestamps: true }) export class Post {
  @Prop({ type: Types.ObjectId, ref: 'User', required: true, index: true }) authorId!: Types.ObjectId;
  @Prop({ type: [String], required: true, validate: [(v: string[]) => v.length > 0 && v.length <= 10, '1-10 images required'] }) imageKeys!: string[];
...
@Schema({ timestamps: true }) export class PostLike {
