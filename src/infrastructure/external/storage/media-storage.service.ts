...
export class MediaStorageService {
  ...
  confirmUpload(userId: string, key: string) {
    if (!key.includes(`/${userId}/`)) throw new BadRequestException('Upload does not belong to current user');
    return { key, confirmed: true };
  }
  signedReadUrl(key: string) {
    return { key, url: `/api/v1/media/${key}`, expiresIn: 900 };
  }
}
